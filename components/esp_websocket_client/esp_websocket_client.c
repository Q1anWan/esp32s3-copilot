...
#define WEBSOCKET_TASK_STACK_CAPS (MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)
#else
#define WEBSOCKET_TASK_STACK_CAPS 0
#endif
...
static void esp_websocket_client_task(void *pv)
{
    ...
    xEventGroupSetBits(client->status_bits, STOPPED_BIT);
    client->state = WEBSOCKET_STATE_UNKNOW;
    bool delete_with_caps = client->task_created_with_caps;
    if (client->selected_for_destroying == true) {
        destroy_and_free_resources(client);
    }
#if WEBSOCKET_TASK_STACK_CAPS
    if (delete_with_caps) {
        vTaskDeleteWithCaps(NULL);
    } else
#else
    (void)delete_with_caps;
#endif
    {
        vTaskDelete(NULL);
    }
...
esp_err_t esp_websocket_client_start(esp_websocket_client_handle_t client)
{
    ...
    const char *task_name = client->config->task_name ? client->config->task_name : "websocket_task";
    BaseType_t task_created = pdFAIL;
    client->task_created_with_caps = false;

#if WEBSOCKET_TASK_STACK_CAPS
    task_created = xTaskCreateWithCaps(esp_websocket_client_task, task_name,
                                       client->config->task_stack, client,
                                       client->config->task_prio, &client->task_handle,
                                       WEBSOCKET_TASK_STACK_CAPS);
    if (task_created == pdTRUE) {
        client->task_created_with_caps = true;
        ESP_LOGI(TAG, "WebSocket task stack allocated in PSRAM");
    } else {
        ESP_LOGW(TAG, "Failed to create websocket task with PSRAM stack, trying internal stack");
    }
#endif

    if (task_created != pdTRUE) {
        task_created = xTaskCreate(esp_websocket_client_task, task_name,
                                   client->config->task_stack, client,
                                   client->config->task_prio, &client->task_handle);
    }
