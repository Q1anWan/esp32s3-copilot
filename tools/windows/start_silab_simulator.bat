...
echo.
echo Sending one logic trigger pulse to %BRIDGE_IP%:%BRIDGE_PORT% ...
py -3 tools\silab_tcp_simulator.py --host %BRIDGE_IP% --port %BRIDGE_PORT% --scene common --seq left_rear_vehicle_merge --rate-hz 10 --pre-idle 1 --hold 2 --post-idle 1 --read-ack --verbose
echo.
